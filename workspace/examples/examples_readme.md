...
このフォルダには 15 個のサンプルプロジェクトが含まれています。
...
| 🎲 **`ticket_assignment_advanced/`** | **確率的動的割当 (全部盛り)** | **complex** | **4,469** | **15** | **10** | **50 名×200 件、マルチスキル・ス��ーミング・不確実性・DAG 依存・CVaR** |
...
## 🎲 ticket_assignment_advanced/ — 確率的動的チケットアサイン (全部盛り)

**50 名のエンジニア × 200 件のチケット × 4 タイムステップの全部盛りチケットアサイン最適化。**

`ticket_assignment/` (20名×80件, medium) の上位版。不確実性とマルチスキルを軸に、
エスカレーション・依存 DAG・非線形 SLA ペナルティ・組織制約を全て盛り込んだ問題。

### データ
- `data/engineers.csv` — エンジニア 50 名 (L1×20, L2×16, L3×10, L4×4)
- `data/tickets.csv` — チケット 200 件 (マルチスキル 39 件、スワーミング対象 28 件)
- `data/ticket_dependencies.csv` — 依存 DAG 24 エッジ (5 グループ: LDAP/ネットワーク/DB/セキュリティ/クラウド)
- `data/constraints.csv` — HC×15, SC×10
- `data/resolution_history.csv` — 過去実績 1,270 件 (分布推定用、マルチスキル・スワーミング実績含む)
- `data/escalation_rules.csv` — 5 ルール (L1→L2→L3→L4、引き継ぎコスト付き)
- `data/sla_penalties.csv` — 非線形 3 段階ペナルティ (warning/violation/critical) + VIP 乗数
- `data/time_periods.csv` — T0-T3 (8 時間、シフト交代、到着予測)
- `data/team_constraints.csv` — 禁止ペア 3、メンタリング 3、チーム維持 2、VIP 指名 2、新人制限 1
- `data/scenario_params.json` — 不確実性パラメータ (対数正規/幾何/ポアソン分布、CVaR 設定)

### 優先要素 1: マルチスキル & スワーミング
- **39 件が 2 スキル以上要求** (うち 8 件が 3 スキル)
- **28 件がスワーミング対象** (lead/support/observer の役割分担)
- P1 障害にはチーム投入で **35-50% の時間短縮**、ただし人的コスト増
- HC11: スワーミングメンバー全体で必要スキルをカバー

### 優先要素 2: 不確実性
- 解決時間が **対数正規分布** (mean + std + confidence)
- ブロック解除が **幾何分布** (ベンダー待ちは T0 で 10%/期、承認待ちは 30%/期)
- 新チケット到着が **ポアソン過程** (T0: P1×1件+P2×2件、T2 以降は減少)
- **CVaR(95%)** で最悪 5% シナリオの SLA 違反数を最小化

### HC (15 個)
HC01-04: 基本制約 (スキル/ティア/シフト/容量) / HC05-06: スワーミングロール / HC07: 依存順序 / HC08: エスカレーション冷却期間 / HC09: 禁止ペア / HC10: L4 は P1+VIP のみ / HC11: マルチスキルカバレッジ / HC12: スワーミング同時参加上限 / HC13: 夜勤帯最低人数 / HC14: VIP 応答時間 / HC15: エスカレーション上位ティア

### SC (10 個)
SC01: SLA 優先 (非線形) / SC02: スキル適性 / SC03: 負荷均等 (疲労加重) / SC04: 再アサイン最小化 / SC05: ティア効率 / SC06: **不確実性ロバスト (CVaR)** / SC07: メンタリングペア / SC08: 依存グループ集約 / SC09: **将来容量確保** / SC10: 疲労回避

### 見どころ
- **不確実性が解の質を支配する**: 確定的最適化では「平均で OK」だが最悪ケースで SLA 違反多発
- **T2 (18:00) の崖**: 人数 34→14 に激減。T0 の段階で P1/P2 を片付けないと夜間に破綻
- **LDAP 障害の連鎖効果**: root cause (TK005) 解決で blocked 5 件が一気に動き出す。優先度計算が変わる
- **スワーミング vs ティア効率のジレンマ**: 3 人投入すれば 50% 時短だが L3 を消費。温存するか投入するか
- **疲労度 × 不確実性の相互作用**: 疲れたベテランより元気な中堅の方がロバストな場合がある

### 参照テンプレート
- `reference/ticket_assignment_template.py` — 基本版 (シングルスキル、確定的)
- `reference/ticket_assignment_guide.md` — ITSM ドメインガイド
